...
public interface BlogRepository extends JpaRepository<Blog,Long> {
    ...
    @Query(value = "SELECT b.*, COUNT(c.id) AS comment_count FROM blog b LEFT JOIN comment c ON b.id = c.blog_id AND c.deleted_at IS NULL WHERE b.deleted_at IS NULL GROUP BY b.id ORDER BY comment_count DESC",  nativeQuery = true)
    List<Blog> getBlogsWithMostComments();

    @Query(value = "SELECT b.*, COUNT(c.id) AS comment_count FROM blog b LEFT JOIN comment c ON b.id = c.blog_id AND c.deleted_at IS NULL WHERE b.deleted_at IS NULL GROUP BY b.id ORDER BY comment_count ASC",  nativeQuery = true)
